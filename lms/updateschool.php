<?php
    ...
    if(isset($_POST['updateschool'])){
        ...
        $boardNoPost = $POST['boardNo'];
      
        include('reusables/connection.php');
        $query = "UPDATE INTO schools SET `Board` = $boardname, `Language` = $language, `School Type` = $schooltype
        WHERE `Board No` = $boardNoPost";
      ...
      <div class="row">
        <div class="col-md-6">
          <form action="updateschool.php" method="POST">
            <input type="hidden" value="<?php echo $_GET['boardNo'] ?>">
            <div class="mb-3">
              <label for="boardname" class="form-label">Board Name</label>
              <input type="text" class="form-control" name="boardname" id="boardname" value="<?php echo $result['Board'] ?>" aria-describedby="boardname">
            </div>

            <div class="mb-3">
              <label for="language" class="form-label">Language</label>
              <input type="text" class="form-control" name="language" id="language" value="<?php echo $result['Language'] ?>"  aria-describedby="language">
            </div>

            <div class="mb-3">
              <label for="schooltype" class="form-label">School Type</label>
              <input type="text" class="form-control" name="schooltype" id="schooltype" value="<?php echo $result['School Type'] ?>" aria-describedby="schooltype">
            </div>
